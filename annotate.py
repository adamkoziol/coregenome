...
class Annotate(object):
    ...
    def coregroups(self):
        while True:
            ...
            if not os.path.isfile(sample.prokka.cds):
                with open(sample.prokka.cds, 'wb') as cds:
                ...
                for record in SeqIO.parse(open(sample.prokka.cds, 'rb'), 'fasta'):
                    #
                    self.cdsparse(record)
    ...
    def combinealleles(self, alleles):
        ...
        if not os.path.isfile('{}/core_combined.tfa'.format(self.coregenelocation)):
            with open('{}/core_combined.tfa'.format(self.coregenelocation), 'wb') as combinedfile:
